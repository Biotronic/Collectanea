module biotronic.DCC.util.flags;

import biotronic.DCC.util.staticparallelstuff;
...
struct Flags(string s, bool generateAll = true) if ( isValidFlagEnumBody!s && parsedString!s.length <= 64 ) {
	...
	static if (generateAll) {
		private alias Representation = uintByBitLength!(memberNames.length);
	} else {
		private alias Representation = OriginalType!MemberEnum;
	}
	...
	bool opBinaryRight(string op : "in")(Flags other) const {
    ...
    assert(i == 0x80000000);
    WsExStyle a = WsExStyle.Popup | WsExStyle.SysMenu;
    assert(WsExStyle.Popup in a);
    assert(WsExStyle.SysMenu in a);
    assert(a in a);
}